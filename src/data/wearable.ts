...
export const wearable: CaseStudy = {
  ...
  dek: "A phone-and-Watch POC. A small on-device model reads notifications and calendar text, extracts 7–10 structured attributes on the phone, and the Watch plays them back as cards that change with the life of the event.",
  lenses: {
    backend: {
      ...
      frame:
        "The model runs on the phone. The interesting system is the contract between extraction and the Watch: 7–10 attributes, five scenarios, one listener path. It’s a POC — that distinction stays on this page.",
      stackLead: ["On-device LLM", "Background processing"],
    },
    android: {
      dek: "A Wear OS piece: listener services, lifecycle-aware cards with 3–5 states, and an on-device model that has to finish before the wrist drops.",
      frame:
        "The wrist story: cards that change from booking to day-of to post-event, carried by Wear OS listener services. The LLM is a means; the demo is a state machine a human can tap.",
  ...
  built: [
    "An Android and Wear OS POC that reads notifications and calendar events with a lightweight on-device LLM, and extracts 7–10 meaningful attributes from unstructured text.",
    "Domain-specific training data and an optimized on-device model tuned for event extraction — 30% lower latency than regex and generic-model baselines.",
    "An end-to-end phone-to-Watch pipeline across five demo scenarios, syncing extracted event data through Wear OS listener services.",
    "Lifecycle-aware Wear OS cards with 3–5 states per event type — status, content, and action buttons that change from booking confirmation to day-of reminder to post-event.",
  ],
  hardParts: [
    {
  ...
  outcomes: [
    "The POC showed a complete loop: notification in, structured event out, Watch card that knows what stage of the event it’s in. Proof of concept, not a shipped Galaxy feature — that distinction stays on this page.",
  ],
  stack: [
    "Kotlin",
    "Android",
