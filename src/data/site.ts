...
export const site = {
  ...
  resumes: {
    backend: { label: "Resume", href: "/resumes/backend.pdf" },
    android: { label: "Resume", href: "/resumes/android.pdf" },
  },
  nav: [
    { to: "/", label: "Selected work" },
    { to: "/projects", label: "Independent" },
...
const pieces: Piece[] = [
  {
    ...
    editions: {
      backend: {
        ...
        dek: "Ten services on a live health platform. A GraphQL BFF, retry-safe writes, and an SQS→Parquet pipeline that doesn’t drop records — 45% fewer client calls, no flag day for REST.",
        lead: true,
      },
      android: {
        no: "05",
        kicker: "Samsung · APIs the clients call",
        dek: "I owned the contracts a screen actually hits — GraphQL BFF and REST — so a page is one query, not a fan-out across ten services.",
      },
    },
  },
  {
    ...
    editions: {
      backend: {
        ...
        dek: "The backend story is the hop I removed — frequent phone↔Watch commands go direct, not through the cloud. About 80% less latency; success from 92% to 98%.",
      },
      android: {
        no: "01",
        kicker: "Samsung · Android / Wear / XR",
        dek: "Device-to-device Find My Mobile on Watch, then the same product shipped on Android XR glasses — from a blank page.",
        lead: true,
      },
    },
  },
  {
    ...
    editions: {
      backend: {
        ...
        dek: "Unstructured events in, 7–10 attributes out. An on-device LLM on the phone — 30% lower extraction latency than regex or generic models.",
      },
      android: {
        no: "02",
        kicker: "Samsung · Wear OS POC",
        dek: "A Wear OS card machine: an on-device LLM extracts the event on the phone, Wear listener services carry it, the wrist shows 3–5 states across the event’s life.",
        lead: true,
      },
    },
  },
